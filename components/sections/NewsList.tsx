...
export default function NewsList({ items }: { items: NewsListItemView[] }) {
  const tCommon = useTranslations("Common");
  const years = useMemo(
    () =>
      Array.from(new Set(items.map((item) => item.publishDate.slice(0, 4)))).sort(
  ...
  const filtered = activeYear
    ? items.filter((item) => item.publishDate.startsWith(activeYear))
    : items;
