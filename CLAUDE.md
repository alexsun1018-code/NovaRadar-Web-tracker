...
- **ValueProps 區塊**（`components/sections/ValueProps.tsx`）：移除「Why NovaRadar」標題（`ValueProps.title`），原副標題升級為 `h2` 主標題並放大（`text-3xl sm:text-4xl lg:text-5xl`）。`ValueProps.title` 翻譯 key 保留未刪除但不再顯示
...
- [ ] `messages/zh-tw.json` 的 `WeInvestIn.items.*.label` 三項標籤（Market Gap／Technology Innovation／Domain Expertise）同樣未翻譯，繁中版顯示英文
